...
<!-- 感觉近些年前端页面（UI）的复杂度增长似乎放缓，与前端开始卷后端了。 -->

2023 年的这张图片把很多人都震惊了，甚至火出了前端圈。  
_这不又违反了关注点分离思想么？_
...
比如在 next.js 中，客户端的函数调用 `getData()` ，会被自动换成 http 请求，最终执行服务端对应的函数，相当于内置了 RPC 能力；  
消除了传统 restful api 的模板代码（客户端的 fetch、服务端的 router），简化了客户端-服务端交互。
...
const data = await getData(42);
```

客户端-服务端的界限不再那么明显，不同端运行的代码文件将被放在同一个项目中、同一个目录下，且可以相互 import。  
_魔法般的力量由编译器提供_

未来是否会被大量开发者接受，拭目以待吧。

<!-- “自 RSC 发布以来，混合客户端/服务器图的思维负担一直是我最大的保留意见” Vue.js 创建者 Evan You -->
