...
- 分享 URL 链接（或二维码），即可**远程抓包记录、Mock 数据**，协助他人或调试移动设备
...
变换后（增加 URL 前缀 `ff-`、HTTP Header `x-ff-proxy-id`）请求会转到代理服务
...
代理服务作为远程中心节点，跟客户端使（UI）用 WebSocket 保持通信，按照 `ff-proxy-id` 隔离。
