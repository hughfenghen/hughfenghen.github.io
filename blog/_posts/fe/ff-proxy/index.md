...
- 分享 URL 链接（或二维码），即可远程共享抓包记录、Mock 规则，协助他人或远程调试移动设备
- 用户可远程相互共享代理规则，无需手动导入导出
...
该方案改进了工作中，高频对特定应用（公司产品的域名、APP） 进行网络调试场景的体验和效率。
...
集成即是将代理服务对应的 SDK 注入到业务系统（网页或 APP）中，**SDK 会拦截并转发请求**到代理服务。

集成（或注入 SDK）的方式介绍以下两种，更多拦截注入代码的方式，请阅读 [Web 终极拦截技巧（全是骚操作）][3]。

### 网关自动注入 SDK

1. 配置 DNS 使 `ff-*` 指向代理服务，代理转发 html 请求时自动注入 SDK
   - 如 Node.js 向 HTML 注入脚本 `resp.body.replace('<head>', '<head><script src="/ff-sdk.js"></script>')`
2. 假设目标域名为 `live.bilibili.com`，使用 `ff-live.bilibili.com` 访问代理服务即可自动注入

该方式无需业务项目对接，适合多个域名需要接入的场景
...
若 URL 参数包含 `ff-proxy-id` 即启动拦截转发，参数值是随机生成 id，用于隔离不同用户的抓包数据、Mock 规则。

拦截转发的方式是重写 `fetch, xhr, WebSocket` 等对象，将请求转到代理服务，请求的变换规则请看下文**代理协议**；  
...
向网络库注册一个拦截器，扫描特制的二维码后，拦截器开始转发请求。  
此外，扫码后 APP 中还需要修改 WebView 容器的 URL，添加前缀（`ff-`）和 Query 参数，前缀和参数的内容在二维码中获取。

## HTTP 请求变换协议

SDK 拦截 HTTP 请求，转到代理服务，然后还原请求再转到目标服务，其关键在于变换与还原请求的协议。
...
代理服务移除 URL 中的前缀，即还原成原始请求，发送给目标服务；  
同时，根据 `x-ff-proxy-id` 将该请求同步给对应 UI 界面，因为代理服务是一个中心节点，需按照 `x-ff-proxy-id` 隔离不同用户的抓包数据、Mock 规则。

## 代理服务与 UI 交互

代理服务是一个远程的中心节点，跟客户端（UI）使用 WebSocket 保持通信，按照 `ff-proxy-id` 隔离。

通过 URL `ff-proxy.bilibili.com/ui/<ff-proxy-id>` 即可访问 UI 界面。

所以实现远程抓包、Mock 是非常自然的，传统代理工具难以实现是因为它工作在用户的设备上。

![http list](./http-list.png)

### 抓包

代理服务每次代理请求都会根据 `ff-proxy-id` 的值，将请求内容 copy 一份发送给客户端，用于调试。

比如，手机上访问接口 `curl -v 'http://ff-api.live.bilibili.com/test' -H 'x-ff-proxy-id: xxx'`；  
...
代理服务还能接受客户端提交的 Mock 规则，在转发请求前、收到响应后，按 Mock 规则篡改请求内容。

Mock 规则 (后称 DSL) 借鉴了 [whistle][2] 并做了大量简化，同时提供 UI 配置（最终会转换成 DSL）。
...
1. 定义 DSL 语法
2. 使用 [peggy][4] 将 DSL 解析为 AST（抽象语法树）
3. 将 AST 转换为 js 函数实现，
4. 函数会检测请求是否命中规则，命中后按规则描述篡权请求内容

**规则存储**

规则存储在本地 indexeddb 中，每次连接时将 DSL 同步到代理服务进行解析；  
所以，代理服务是一个干净的、只维持临时状态的服务，方便部署与扩展。

## 相对传统方案的缺点

方案的优点写在了文章开头，这里有必要介绍一下缺点，感兴趣的同学自行斟酌。

- 需要部署、维护代理服务
  - 该方案将以系统维护者的一次性部署成本，交换用户安装、配置软件的成本
- 侵入性
  - 该方案不是一个透明的中间节点，对业务系统有一定侵入性
  - DNS 集成方式，可让公司系统无感自动接入
- 通用性
  - 该方案无法调试系统网络请求，无法调试未接入的业务系统、APP

总结来说，实施该方案的前提是：团队足够大，（效率、体验改进的）**收益**大于**付出**（代理系统部署维护）。
