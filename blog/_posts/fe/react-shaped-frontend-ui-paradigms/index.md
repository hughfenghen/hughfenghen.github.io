...
那时很多开发者看 React 的 jsx 语法（混合 html/js）多少有点**大逆不道**，因为它违反了**关注点分离**的设计思想。
...
如果前端 UI 与后端的表层服务是同一个团队维护，在客户端中（不限于组件内）直接调用服务端函数确实是非常方便的。

比如在 Next.js 中，客户端的函数调用 `getData()` ，会被自动转换成 http 请求，最终执行服务端对应的函数，相当于内置了 RPC 能力；  
...
客户端/服务端泾渭分明的界限不再那么明显，分别在浏览器与服务器中运行的代码文件将被放在同一个项目中、同一个目录下，且可以相互 import，如同魔法一般。

为 UI 提供数据的表层后端接口**通常跟特定业务强相关**，通常无法（无需）被复用，把相关代码放在同一个目录下，相对前后端分离是有明显简化的；  
...
> 螺旋形上升(spiral rise) 是对否定的否定规律所揭示的事物发展形式的一种形象比喻。
...
关注点分离的思想并没有错，否则 Vue、Svelte 也不会发展出如此繁荣的生态；  
Vue 在单文件组件中融合了 html/js/css，同时又保持了关注点分离；  
相比 React（jsx）融合 html/js 而丢掉 css，然后用各种 css in js 方案来找补，从这个维度来看是略逊于 Vue 的。

我总结几次**范式革命背后的驱动力是亲密性原则**，规模（复杂性）的增长让代码逐渐违反了亲密性原则。

为实现共同目标的代码应该放在一起，规模较大的模块需要拆分成更细粒度；  
但关系亲密、相互合作的代码就应该让它们的物理距离（代码位置）更接近，遵循亲密性原则；  
如果它们的天赋属性（如 html/js, 前/后端）有区别，那就用新技术去融合或拉进它们距离，而不是用来分割它们理由。
