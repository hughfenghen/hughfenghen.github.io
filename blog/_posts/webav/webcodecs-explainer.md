...
它们这样做可能是因为当前开放的 Web API 在某些场景都存在难以克服的障碍:
- WebAudio 允许将媒体文件(以二进制缓冲区的形式)解码为 PCM，但它需要是一个有效且完整的媒体文件。它不支持数据流，不提供解码的进度信息。当然，也不支持解码视频，更不支持编码。
- MediaRecorder 允许对具有音频和视频轨道的 MediaStream 进行编码。可以粗略地控制一些参数(比特率、带编解码器字符串的 mimetype)，但它是非常高层次的。它不支持超过现实速度来快速编码；由于输出可能被缓冲，因此不适合低延迟编码。编码比特流被封装在一个容器中，这为需要自定义容器格式的使用场景增加了开销（译注：MediaRecorder 只能输出 WebM 格式的视频，不支持其他流行的视频封装格式，如 MP4）。许多事情不透明：数据不足时会发生什么，编码速度太慢而无法实时时会发生什么。它对于基础应用非常友好，但缺少许多功能。（译注：总结来说，这是一个高层次的 API，无法做到偏底层的精细控制）
